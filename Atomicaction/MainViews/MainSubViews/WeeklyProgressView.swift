...
struct WeeklyProgressView: View {
    ...
    var weekDateRangeString: String {
        ...
        guard let firstDay = days?.first, let lastDay = days?.last else {
            ...
            let startString = firstDay.formatted(.dateTime.month(.abbreviated).day())
            
            // If start and end are in the same month, show: "Aug 17 – 23"
            if Calendar.current.isDate(firstDay, equalTo: lastDay, toGranularity: .month) {
                let endDayOnly = lastDay.formatted(.dateTime.day())
                return "\(startString) – \(endDayOnly)"
            } else {
                // Across months: "Aug 31 – Sep 6"
                let endString = lastDay.formatted(.dateTime.month(.abbreviated).day())
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            Label("Actions — Week, \(weekDateRangeString)", systemImage: "calendar")
            ...
            Chart(weeklyBuckets) { bucket in
                ...
                .foregroundStyle((bucket.dayRatio > 4 ? .green :
                ...
                AxisMarks { value in
                    AxisValueLabel {
                        if let label = value.as(String.self) {
                            ...
                            Text(label)
                                .foregroundStyle(isToday ? .indigo : Color.secondary)
...
extension Calendar {
    func currentWeekDays() -> (first: Date, last: Date)? {
        var mondayCalendar = self
        mondayCalendar.firstWeekday = 2
        
        guard let weekStart = mondayCalendar.dateInterval(of: .weekOfYear, for: Date())?.start,
              let weekEnd = mondayCalendar.date(byAdding: .day, value: 6, to: weekStart) else { return nil }
