...
struct RoutineHomeView: View {
                ...
                if let compDate = routine.completeDate,
                   !Calendar.current.isDate(compDate, inSameDayAs: Date()){
                    ...
                    if !routine.routines.isEmpty {
                        routine.routines.forEach(){ $0.doneToday = false }
    ...
    private var content: some View {
        ...
        case .routineHome:
            if let routine = currentRoutine, !routine.routines.isEmpty {
                
                if let compDate = currentRoutine?.completeDate, Calendar.current.isDate(compDate, inSameDayAs: Date()) {
                    compScreen
...
//                    .zIndex(0)
        ...
        case .editRoutine(routine: let routine):
            EditRoutineView(routine: routine, router: router)
        case .routineSession(routine: let routine):
            RoutineSessionView(routine: routine, router: router)
                //.transition(.move(edge: .trailing)) 
                .toolbar(true ? .hidden : .visible, for: .tabBar)
//                .transition(.asymmetric(
...
//                        .font(.system(size: 60))
//                        .foregroundStyle(.tint)
//                    Text("Build a routine made of tasks\nthat add up to exactly 60 minutes.")
//                        .multilineTextAlignment(.center)
//                        .foregroundStyle(.secondary)
//                    Spacer()
//                    NavigationLink("Create 1-Hour Routine") {
//                        CreateRoutineView()
//                    }
//                    .buttonStyle(.borderedProminent)
//                    .controlSize(.large)
